...
total_kanji = sum(len(g['items']) for g in groups)
print(f"Total kanji parsed: {total_kanji}")

# Check mismatches
for i, g in enumerate(groups):
    if len(g['items']) != g['expected_count']:
        print(f"Group {i+1} ({g['title']}): expected {g['expected_count']}, got {len(g['items'])}")
